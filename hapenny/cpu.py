...
from amaranth_soc.wishbone.bus import Arbiter, Interface
...
class Cpu(Component):
    ...
    def __init__(
        self, *, reset_vector=0, addr_width=31, counters=False, prog_addr_width=None
    ):
        super().__init__()
        ...
        self.bus: Interface = WBSignature(
            addr_width=addr_width, data_width=16, granularity=8
        ).create()

        self.arb = Arbiter(addr_width=addr_width, data_width=16, granularity=8)
        self.dec = WBDecoder(addr_width=addr_width, data_width=16, granularity=8)
    ...
    def elaborate(self, platform):
        ...
        # Update to Wishbone.
        # this may need to be a arbitrator.

        # Combine the bus access ports. The debug port can't drive our bus, so
        # this is simpler.
        # if this is we behaved  split states it does not need an arbiter
        ...
        # m.d.comb += [
        #     self.bus.cyc.eq(fd.bus.cyc | ew.bus.cyc),
        #     self.bus.stb.eq(fd.bus.stb | ew.bus.stb)
        # ]
        # m.d.comb += [
